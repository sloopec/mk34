...
- Pro Szene ein Lauf-Protokoll: gewaehltes Modell (lokal/Cloud), Routing-Begruendung, Editor-Iterationen, Continuity-Konflikte, Token-/Zeitverbrauch — als JSON-Zeile neben dem Manuskript
- Kosten-Tracking pro Szene: Cloud-Tokens vs. lokale Laufzeit
- **Judge-Kosten pro Eval-Lauf, getrennt nach Stufe** (`fast` / `craft`) — dient der Validierung von E4: ist die Aufteilung richtig geschnitten, oder muesste eine Rubrik umgestuft werden?

**Ausbauvariante (ausgeschlossen im aktuellen Umfang):**
Cloud Trace, Prompt-Response-Logging, BigQuery Agent Analytics (`BigQueryAgentAnalyticsPlugin`) setzen ein GCP-Projekt voraus. Mit E1 (kein GCP) und dem Deployment-Non-Goal entfaellt das.
