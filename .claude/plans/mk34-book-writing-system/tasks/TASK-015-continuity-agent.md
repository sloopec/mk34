...
**Entscheidung E2: Retrieval laeuft lokal.** Kein Vertex AI Vector Search, keine Cloud-Ingestion — der Stoff verlaesst die Maschine nicht (dieselbe Begruendung wie fuer die verschluesselte VM des lokalen Modells), und der Korpus ist klein (ein Roman ≈ 200–400 Szenen-Chunks).
...
**Agent:**
- Modell: `model_for("continuity")` → `anthropic/claude-sonnet-5`
- Tool: `check_consistency(new_text: str, context_refs: list[str]) -> dict` mit `{"conflicts": [{"type", "evidence", "reference", "severity"}]}`
- Einbindung als `AgentTool` in der Pipeline (vor dem Editor-Loop)
...
- [ ] Die Pipeline bricht bei `severity: high` ab, statt die Szene zu schreiben
